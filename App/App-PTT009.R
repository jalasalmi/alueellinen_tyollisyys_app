...
ui <- fluidPage(
  ...
  sidebarLayout(
    ...
    sidebarPanel(#checkboxInput("kokomaa", "Koko maa", value = TRUE),
                 ...
                 checkboxInput("eteläkarjala", "Etelä-Karjala"),
                 
                 conditionalPanel(
                   condition = "input.eteläkarjala == true",
                   checkboxGroupInput("kunta_sel_eteläkarjala", "Valitse kunta",
                                      choices = unique(data_kunnat$alue[data_kunnat$Maakunta == "Etelä-Karjala"]))),
                 
                 checkboxInput("eteläpohjanmaa", "Etelä-Pohjanmaa"),
                 
                 conditionalPanel(
                   condition = "input.eteläpohjanmaa == true",
                   checkboxGroupInput("kunta_sel_eteläpohjanmaa", "Valitse Kunta",
                                      choices = unique(data_kunnat$alue[data_kunnat$Maakunta == "Etelä-Pohjanmaa"]))),
                 
                 checkboxInput("eteläsavo", "Etelä-Savo"),
                 
                 conditionalPanel(
                   condition = "input.eteläsavo == true",
                   checkboxGroupInput("kunta_sel_eteläsavo", "Valitse Kunta",
                 ...
                 checkboxInput("kantahäme", "Kanta-Häme"),
                 
                 conditionalPanel(
                   condition = "input.kantahäme == true",
                   checkboxGroupInput("kunta_sel_kantahäme", "Valitse Kunta",
                 ...
                 checkboxInput("päijäthäme", "Päijät-Häme"),
                 
                 conditionalPanel(
                   condition = "input.päijäthäme == true",
                   checkboxGroupInput("kunta_sel_päijäthäme", "Valitse Kunta",
...
server <- function(input, output, session) {
  ...
  kunta_sel <- reactive({
    ...
    kunta_sel <-  c(input$kunta_sel_ahvenanmaa, input$kunta_sel_eteläkarjala, input$kunta_sel_eteläpohjanmaa, 
                    input$kunta_sel_eteläsavo, input$kunta_sel_kainuu, input$kunta_sel_kantahäme, 
                    ...
                    input$kunta_sel_pohjoissavo, input$kunta_sel_päijäthäme, input$kunta_sel_satakunta,
  ...
  maakunta_sel <- reactive({
    ...
    maakunta_sel <- c(input$ahvenanmaa, input$eteläkarjala, input$eteläpohjanmaa,
                      input$eteläsavo, input$kainuu,
                      input$kantahäme, input$keskipohjanmaa, input$keskisuomi,
                      input$kymenlaakso, input$lappi, input$pirkanmaa,
                      input$pohjanmaa, input$pohjoiskarjala, input$pohjoispohjanmaa,
                      input$pohjoissavo, input$päijäthäme, input$satakunta,
  ...
  # Tämä poistaa kuntavalinnan, mikäli käyttäjä poistaa kyseisen kunnan maakuntavalinnan. 
     observe({ 
       ...
       for(mk in maakunnat) {
          ...
          updateCheckboxGroupInput(session, paste("kunta_sel", 
                                                  tolower(paste(unlist(strsplit(mk, "-")), sep = "", collapse = "")), 
                                                  sep = "_"), 
                                 choices = unique(data_kunnat$alue[data_kunnat$Maakunta == mk]), 
                                 selected = NULL)
            }
  ...
  output$employment_plot <- renderPlot({
    ...
    #if(is.null(maakunta_sel)) {
    #  kunta_sel <- NULL
    #}
    
    # if(!is.null(kunta_sel)) {
    #   if(as.character(data_kunnat$Maakunta[data_kunnat$alue == kunta_sel][1]) != maakunta_sel) {
    #     kunta_sel <- NULL
    #   }
    # }
    
    # if(input$kokomaa) {
    #   data_kokomaa %>% filter(value_type == "Työlliset_trend") %>%
    #                    ggplot(aes(x = time, y = value, group = alue, col = value_type)) +
    #                    geom_line(alpha = 0.5) + 
    #                    ggtitle("Työlliset, koko maa") +
    #                    ylab("Työllisiä, trendi") + 
    #                    xlab(NULL) +
    #                    theme_ptt()
   ...
   # } else 
    ...
    # Piirrä maakuntakuvaaja jos maakunta_sel ei ole tyhjä ja kunta_sel on tyhjä
      ...
      data_maakunnat %>% filter(alue %in% maakunta_sel,
                                value_type =="Työlliset_trend") %>%
        ggplot(aes(x = time, y = value/1000, col = alue)) +
        geom_line() + 
        labs(title = "Työlliset",
             y = paste("1000 henkilöä, ", input$adjustment, sep = ""),
             x = NULL,
             caption = "Lähde: Työvälitystilasto (TEM), Tilastokeskus, PTT") +
        theme_ptt() +
  ...
  output$unemployment_plot <- renderPlot({
    ...
    työttömyysluku <- paste("Työttömät", adjustment(), sep = "")
  ...
  output$unemployment_rate <- renderPlot({
    ...
    työttömyysluku <- paste("Työttömyysaste", adjustment(), sep = "")
    ...
    # Piirrä maakuntakuvaaja jos maakunta_sel ei ole tyhjä ja kunta_sel on tyhjä
      ...
      data_maakunnat %>% 
          mutate(alue2 = fct_other(alue, keep = maakunta_sel,
                 ...
                 alue = fct_relevel(alue, maakunta_sel, after = Inf)) %>%
                         filter(value_type %in% työttömyysluku) %>%
        ggplot(aes(x = time, y = value, group = alue, col = alue2)) +
        scale_color_manual(values = na.omit(c(ptt_pal(length(maakunta_sel)), "grey90"))) +
        geom_line(size = viivapaksuus) + 
      ...
      data_kunnat %>% filter(Maakunta == maakunta_sel) %>%            # tämän filtterin poistamlla kaikki Suomen kunnat 
        mutate(alue2 = fct_other(alue, keep = kunta_sel,               # näkyvät taustalla, set geom_line(size < 0.5)
               ...
               alue = fct_relevel(alue, kunta_sel, after = Inf)) %>%
                      filter(value_type %in% työttömyysluku) %>%
        ggplot(aes(x = time, y = value, group = alue, col = alue2)) +
        scale_color_manual(values = na.omit(c(ptt_pal(length(kunta_sel)), greys[1:length(maakunta_sel)]))) + 
        geom_line(size = viivapaksuus) +
        labs(title = "Työttömyysaste",
             y = paste("%, ", tolower(input$adjustment), sep = ""),
             x = NULL,
  ...
  # Muutos
   ...
   output$change_tyott_aste <- renderPlot({
    ...
    if(!is.null(maakunta_sel) & is.null(kunta_sel)) {
       ...
       group_by(alue) %>%
          select(-Työvoima) %>%
        filter(value_type == "Työttömyysaste_trend") %>%
        spread(value_type, value) %>%
        summarise(t0 = Työttömyysaste_trend[time == as.Date(input$change_period[1])],
                  t1 = Työttömyysaste_trend[time == as.Date(input$change_period[2])]) %>%
        ungroup() %>%
        ...
        labs(title = "Työttömyysaste, trendi",
             x = "%",  caption = "Lähde: Työnvälitystilasto (TEM), Tilastokeskus, PTT") +
   ...
   output$map <- renderPlot({
     ...
     # Tämän aktivoimalla kuntavalinta rajaa kunnan kartasta
